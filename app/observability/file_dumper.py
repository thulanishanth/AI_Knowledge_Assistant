...
SQL_FILE = LOG_DIR / "generated_sql.txt"
...
def _sanitize_for_txt(text: str) -> str:
    """Keep each entry on a single line for line-aligned debug files."""
    if not text:
        return "None"
    return str(text).replace("\n", "\\n").replace("\r", "").strip()


def sync_dump_to_txt(
    user_query: str,
    ai_response: str,
    full_prompt: str,
    rag_context: str,
    ...
    """Synchronously append data to debug dump files."""
    try:
        ...
        clean_status = _sanitize_for_txt(execution_status)
        ...
        logger.error("Failed to dump conversation to txt files: %s", exc)
...
async def dump_conversation(
    ...
    """Async wrapper so file logging does not block the request cycle."""
